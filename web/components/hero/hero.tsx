...
export function Hero({ onExploreClick, onDispatchClick }: HeroProps) {
  return (
    <section className="relative min-h-[75vh] pt-28 pb-20 px-6 sm:px-12 flex flex-col items-start justify-center max-w-5xl mx-auto overflow-hidden bg-cad-grid">
      ...
      {/* Large Technical Monospace Watermark Typography in Background (2-3% opacity for depth without noise) */}
      <div className="absolute right-4 top-1/2 -translate-y-1/2 font-mono font-black text-7xl sm:text-9xl text-white/[0.025] select-none pointer-events-none tracking-tighter leading-none text-right z-0">
        <div>{"// APEX SPEC"}</div>
      ...
      <div className="relative z-10 w-full flex flex-col items-start text-left">
        ...
        {/* Headline */}
        <h1 className="text-5xl sm:text-6xl md:text-7xl lg:text-8xl font-black tracking-[-0.04em] text-white leading-[0.92] mb-6 uppercase select-none">
          DECONSTRUCT<br />
          MOTORSPORT<br />
        ...
        {/* Proof Bar */}
        <div className="pt-6 border-t border-[#192234] flex flex-wrap items-center gap-4 text-xs font-mono text-[#78859B] uppercase tracking-wider w-full">
